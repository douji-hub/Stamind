...
const Sidebar = () => {
  ...
  return (
    <>
      <div
        className={`absolute w-[14rem] h-screen bg-stamind-black-850 flex flex-col justify-between ${sidebarState ? 'animate-sidebarOpen' : 'animate-sidebarClose'}`}
      >
      ...
      <div
        className={`w-[2rem] h-[2.75rem] bg-stamind-specific-background_primary flex flex-col justify-center ${!sidebarState ? 'animate-sidebarOpen' : 'animate-sidebarClose'}`}
      >
        <CloseButton onHandleClick={handleButtonClick} />
      </div>
    </>
